...
myboot <- boot_permute_factory(tibble(x = 1:20))
set.seed(0)
myboot()
...
result_boot <- tibble(
  b = 1 : B,
  permute = map(map(map(b, ~ boot_permute_mtcars()), model_mtcars), tidy)
)
...
boot_parametric_factory <- function(df, model) {
  function() {
    fit <- fitted(model(df))
    res <- resid(model(df))
    df$mpg <- fit + sample(res, length(res), replace = TRUE)
    return(df)
  }
}
...
result_boot <- result_boot %>% 
  mutate(parametric = map(b,~boot_parametric_mtcars())) %>%
...
ci_bootstrap <- result_boot_tidy %>% 
  group_by(boot,term) %>% 
  summarise(basic = map(full[1],~ci_basic_bootstrap(estimate,.x)),
            percentile = map(full[1],~ci_percentile_bootstrap(estimate,.x)) ) %>%
  pivot_longer(cols = c("basic","percentile")) %>% unnest(value)
