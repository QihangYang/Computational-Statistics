...
set.seed(100)
seed2 <- .Random.seed
pvalue_result <- ks.test(df$x, pweibull, shape = 3, scale = 2, alternative = "two.sided")$p.value
pvalue_result
...
g1 <- ggplot(data = df, mapping = aes(x = x)) +
  ...
  stat_function(fun = dweibull, args = list(shape = 3, scale = 2), aes(color = "True value")) +
...
## Do not modify this line!
g2 <- ggplot(data = df, mapping = aes(sample = x)) +
  geom_qq(distribution = qweibull, dparams = list(3, 2)) +
  stat_qq_line(distribution = qweibull, dparams = list(3, 2), color = "red") +
