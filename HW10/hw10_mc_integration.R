...
f_3 <- factory(3)
integral_f_3 <- integrate(f_3, lower = 0, upper = 1)$value
...
ci_anti <- df2 %>%
  summarize(integral = mean(f_3 + f_3_anti) / 2,
            se = sqrt((var(f_3) + var(f_3_anti) + 2 * cov(f_3, f_3_anti)) /4 /n),
            lower = integral - 1.96 * se, 
            upper = integral + 1.96 * se)

ci_anti_m <- df2 %>%
  summarize(integral = mean(f_3 + f_3_anti) / 2,
            se = sqrt(var((f_3 + f_3_anti)/2)),
            lower = integral - 1.96 * se, 
            upper = integral + 1.96 * se)
...
integral_f_2 <- integrate(f_2, 0, 1)$value

df3 <- df2 %>%
  mutate(f_2 = f_2(s))

ci_control <- df3 %>% 
  summarise(c = -cov(f_3, f_2) / var(f_2),
            integral = mean(f_3 + c * (f_2 - integral_f_2)),
            se = sqrt((var(f_3) - cov(f_3, f_2) ^ 2 / var(f_2)) / n),
            lower = integral - 1.96 * se,
            upper = integral + 1.96 * se)
...
errors <- cum_stats %>%
  mutate(sequence = seq(1, n-1, 1),
         unif = cummean_f_3 - integral_f_3,
         antithetic = (cummean_f_3 + cummean_f_3_anti) / 2 - integral_f_3,
         control = cummean_f_3 + cum_c * (cummean_f_2 - integral_f_2) - integral_f_3) %>%
  select(-s, -f_3, -f_3_anti, -f_2) %>%
  select(sequence, unif, antithetic, control) %>%
  pivot_longer(-sequence, names_to = "sample_type", values_to = "value")
...
error_plot <- ggplot(data = errors, mapping = aes(x = sequence, y = value, color = sample_type)) +
  geom_line() +
  coord_cartesian(xlim = c(0, 1500)) +
  labs(y = "MC error",
       colour = "Sample type",
