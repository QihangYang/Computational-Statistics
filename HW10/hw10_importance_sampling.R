...
norm_plot <- ggplot(data)+
  geom_line(aes(x,z))+
  geom_line(filter(data,y==1),mapping=aes(x,z),color="red")+
  labs(x="X",
       y="Density Function",
...
fit <- tribble(
  ...
  mutate(g_over_f = map2(x,f,function(a,b){g(a)/b}))
...
result <- fit %>%
  transmute(
    mean = map_dbl(g_over_f,mean),
    se = map_dbl(g_over_f,sd)/sqrt(n),
    lower = mean - 1.96 * se,
    upper = mean + 1.96 * se)
...
subsamples <- tribble(
  ...
  mutate(g_over_f = map2(width,samples,
...
variance_plot <- 
  ggplot(result2,aes(y = mean, x = width))+
  geom_line()+
