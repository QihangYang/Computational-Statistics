...
F1 <- function(x, location = 0, scale = 1) {
  res <- 1/pi * atan((x - location) / scale) + 1/2
  return(res)
}
F1(0)
F1(1,location=-1,scale=0.5)
...
Finv <- function(u, location = 0, scale = 1) {
  res <- scale * tan(pi * (u - 1/2)) + location
  return(res)
}

Finv(0.5)
Finv(0.92, location=-1, scale=0.5)
...
library(ggplot2)
library(dplyr)
g1 <- ggplot(data = df %>% filter(x > -50 & x < 50), 
             mapping = aes(x = x)) +
  geom_histogram(aes(y = ..density..), bins = 100) +
  stat_function(fun = dcauchy, args = list(location = 0, scale = 1), color = "red") +
