...
coeffs <- c(0, 0, 1, 1)
m <- c(1, 1, 3, 14)
b <- c(2 * coeffs[1] * m[4], 2^2 * coeffs[2] * m[3], 
       2^3 * coeffs[3] * m[2], 2^4 * coeffs[4] * m[1], m[1])
...
next_in_seq <- function(coeffs, previous) {
  aux <- function(x, k){
    return(as.binary(2^k * x))
  }
  tmp <-tibble(coeff_x_previous = coeffs * previous,
         k = seq_along(coeff_x_previous),
         tmp = map2(coeff_x_previous, k, aux)) %>%
    pull(tmp) 
  res <- reduce(tmp, xor)
  res <- xor(res, as.binary(previous[length(previous)]))
  res <- as.numeric(res)
  return(res)
}
...
sobol <- function(coeffs, n, init){
  output <- init
  previous <- init[length(init) : 1]
  while(length(output) < n){
    new <- next_in_seq(coeffs, previous)
    output <- c(output, new)
    previous <- c(new, previous[-length(previous)])
  }
  return(post_process(output))
}
sobol(c(0, 1, 1), 5, c(1, 1, 3))
...
simul_tibble <- tibble(
  ...
  normal = rnorm(50, mean = 0.5, sd = 0.25),
  uniform = runif(50)
) %>%
...
## Do not modify this line!
library(ggplot2)
simul_plot <- ggplot(data = simul_tibble, mapping = aes(x = sequence, y = value)) +
  geom_point() +
  facet_wrap(~type) +
