...
boot_ratio <- function(nboot,df){
  ti <- tibble(b=seq(1,nboot,1)) %>%
    mutate(df_b=map(b, function(b,df) boot_df(df), df=df),
           theta_hat_b=map(df_b,ratio))
  as.numeric(pull(ti))}
...
results <- tibble(nboot=c(50,100,200,500,1000)) %>%
  mutate(theta_hat_b=map(nboot,boot_ratio,df=df)) %>%
  mutate(m=map(theta_hat_b,mean),bias=as.numeric(m)-theta_hat,theta_hat_corrected=theta_hat-bias,type='normal')%>%
  select(-m,-theta_hat_b) 
...
boot_p_ratio<-function(df){
  n<-nrow(df)
  s<-sample(seq(1,n,1),n,replace=T)
  p<-tabulate(s,nbins=n)/n
  theta<-rratio(p,df)
...
boot_ratio2<-function(nboot,df){
  t<-tibble(b=seq(1,nboot,1)) %>%
    mutate(df_b=map(b,function(b,df)boot_p_ratio(df),df=df)) %>%
    unnest(cols='df_b') %>%
...
results2<-tibble(nboot=c(50,100,200,500,1000)) %>%
  mutate(theta_hat_p_hat=map(nboot,boot_ratio2,df=df)) %>%
  unnest(cols='theta_hat_p_hat') %>%
  mutate(theta_hat=map_dbl(p_hat_b,rratio,df=df),
         m=map(theta_hat_b,mean),
         bias=as.numeric(m)-theta_hat,
         theta_hat_corrected=theta_hat-bias,
         type='improved')%>%
  select(-m,-theta_hat_b,-p_hat_b,-theta_hat)
...
g1 <- ggplot(data=bind_rows(results2,results),aes(x=nboot,y=bias,color=type))+
  geom_line()+
