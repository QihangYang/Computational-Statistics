...
## Do not modify this line!
density_plot <- ggplot(data = X, mapping = aes(x1, x2)) +
...
## Do not modify this line!
f <- function(x) {
  res <- ddirichlet(c(x[1], x[2], 1 - x[1] - x[2]), c(2,2,2))
  return(res)
}
f(c(0,0))
f(c(0.3, 0.3))
...
sample_one <- function() {
  ...
  res <- tibble(x1 = x[1], x2 = x[2], count)
  return(res)
}
...
#    - `theme_light()` to set the theme to light.
## Do not modify this line!
sample_plot <- ggplot(data = samples, mapping = aes(x1, x2)) +
...
## Do not modify this line!
g2 <- function(x){
  res <- dbeta(x[1], 1.1, 1.1) * dbeta(x[2], 1.1, 1.1)
  return(res)
}
g2(c(0, 0))
g2(c(0.5, 0.5))
...
samples2 <- sample_n2(1000) 

acceptance_rate2 <- 1000 / sum(samples2$count)
