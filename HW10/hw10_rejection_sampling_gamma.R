...
mode <- optimize(f, interval = c(0,10), maximum = TRUE)$maximum
mode

M_best <- optimize(f, interval = c(0,10), maximum = TRUE)[[2]]
M_best
...
n <- 1e3
result <- tibble(
  M = seq(M_best, 5, by = 0.025),
  Y = map(M, ~ rejection_sampling(.x, f, g)),
  acceptance_rate = map_dbl(Y, ~ length(.x)/n)
)
result
...
## Do not modify this line!
acceptance_plot <- ggplot(data = result, 
                          mapping = aes(x = M, y = acceptance_rate)) +
  geom_line()+
...
good_rejection_sampling_plot <- ggplot(data = result2, 
                                       mapping = aes(x = y)) +
  geom_point(aes(y = M2 * u * g2(y), color = selected)) +
  stat_function(fun = f , size = 2) +
  stat_function(fun = g3, color = "red", size = 2) +
  labs(x = "x", y = "Density", 
