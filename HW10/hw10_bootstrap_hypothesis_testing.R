...
library(tidyverse)
speed_hist <- ggplot(data = tibble(speed = speed), mapping = aes(speed)) +
...
#    - `theme_light()` to set the theme to light.
## Do not modify this line!
speed_qq <- ggplot(data = tibble(speed = speed), mapping = aes(sample = speed)) +
  stat_qq() +
  stat_qq_line() +
...
bstrap <- tibble(
  b = 1 :1000,
  sample_null = map(b, ~ boot_speed_null()),
...
#    - `theme_light()` to set the theme to light.
## Do not modify this line!
bstrap_hist <- ggplot(data = bstrap, mapping = aes(muhat_null)) +
  geom_histogram(bins = 30, color = "white") +
...
boot_pval <- function(muhat, mu0, muhat_null){
  n <- length(muhat_null)
  a <- sum(abs(muhat_null - mu0) > abs(muhat - mu0))
  p <- a / n 
  return(p)
}

boot_pval(30, 31, speed)

p_value <- boot_pval(muhat = mean(speed), mu0 = 33.02, muhat_null = bstrap$muhat_null)
...
newspeed_qq <- ggplot(data = tibble(speed = newspeed), mapping = aes(sample = speed)) +
  stat_qq() +
  stat_qq_line() +
...
boot_speed_null2 <- function(){
  newspeed <- newspeed + 33.02 - mean(newspeed)
  sample(newspeed, size = length(newspeed), replace = TRUE)
}
...
bstrap2 <- tibble(
  b = 1 : 1000,
  sample_null = map(b, ~ boot_speed_null2()),
  muhat_null = map_dbl(sample_null, mean)
)
bstrap2

p_value2 <- boot_pval(muhat = mean(newspeed), mu0 = 33.02, muhat_null = bstrap2$muhat_null) 
