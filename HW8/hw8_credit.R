...
## Do not modify this line!
g2 <- ggplot(data = credit_new, mapping = aes(x = Limit_per_card, y = Balance, col = Student)) +
  geom_point() +
  geom_smooth() +
...
g3 <- ggplot(data = credit_new, mapping = aes(x = log_limit_per_card, y = Balance, col = Student)) +
  geom_point() +
  geom_smooth() +
...
## Do not modify this line!
library(modelr)
m1 <- lm(data = credit_new, Balance ~ log_limit_per_card * Student)
m1  %>% broom::tidy()

credit_pred <- credit_new %>%
...
g32 <- credit_pred %>% 
  ...
  ggplot(aes(x = log_limit_per_card, y = Balance, color = Student)) +
  geom_point() +
  geom_smooth(aes(linetype = Type), se = F, size = 0.5) +
...
## Do not modify this line!
g4 <- ggplot(data = credit_pred, mapping = aes(x = Balance, y = resid1)) +
  geom_ref_line(h = 0) +
  geom_point() +
...
g5 <- ggplot(data = credit_pred) +
  geom_smooth(mapping = aes(x = Rating, y = pred1, col = "Predictions"), se = FALSE) +
  geom_smooth(mapping = aes(x = Rating, y = Balance, col = "Data"), se = FALSE) +
  geom_point(mapping = aes(x = Rating, y = Balance)) +
...
## Do not modify this line!
m2 <- lm(data = credit_pred, Balance ~ log_limit_per_card * Student + Rating) 
m2 %>% broom::tidy()
...
g7 <- ggplot(data = credit_pred) +
  ...
  geom_smooth(aes(x = Rating, y = Balance, col = "Data"), se = FALSE) +
  geom_smooth(aes(x = Rating, y = pred1, col = "Predictions 1"), se = FALSE) +
  geom_smooth(aes(x = Rating, y = pred2, col = "Predictions 2"), se = FALSE) +
...
## Do not modify this line!
m3 <- lm(data =credit_pred, Balance ~ log_limit_per_card * Student + Rating + Income + Limit + Cards)
m3 %>% broom::tidy()
...
library(purrr)
f1 <- function(df){
  res <- lm(data = df, Balance ~ log_limit_per_card * Student + Rating + Income + Limit + Cards)
  return(res)
}
...
g9 <- ggplot(data = credit_model, mapping = aes(x = Balance, y = resid)) +
  ...
  geom_smooth(aes(col = Ethnicity), se = FALSE, method = "lm") +
...
credit_model_glance <- credit_model %>%
  nest(resid = -c(Ethnicity, data, model)) %>%
