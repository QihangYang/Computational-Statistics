...
## Do not modify this line!
library(modelr)
model_quad <- lm(data = train_data, y ~ poly(x, 2, raw = TRUE))
data_with_quad_resid <- all_data %>%
  add_residuals(model_quad)
...
quad_resid_plot <- ggplot(data = data_with_quad_resid, mapping = aes(x = x, y = resid)) +
  geom_ref_line(h = 0) +
  geom_point() +
  labs(title = "The residuals show a secondary pattern that is not captured",
       y = "Residuals") 
quad_resid_plot
...
library(tidyr)
my_model <- function(k){
  res <- lm(data = train_data, y ~ poly(x, k, raw = TRUE)) 
}
...
## Do not modify this line!
plot_mse <- ggplot(data = model_mse, mapping = aes(x = k, y = mse, col = split)) +
  geom_line() +
  scale_x_continuous(breaks = 1:10) +
  labs(title = "Train and test error vary with the polynomial degree",
       subtitle = "Test error first decreases, then increases",
       x = "Polynomial Degree",
       y = "RMSE")
plot_mse
...
## Do not modify this line!
t9 <- filter(model_pred_resid, k == best_deg)
residual_plot <- ggplot(data = t9) +
  geom_point(mapping = aes(x = x, y = resid)) +
  geom_abline(mapping = aes(intercept = 0, slope = 0), size = 1) +
  labs(title = "No clear pattern in the residuals",
       subtitle = "The model did its job!",
       y = "Residuals")
residual_plot
...
## Do not modify this line!
t10 <- model_pred_resid %>%
  filter(k == 7 | k == 9) %>%
  mutate(k = factor(k))

prediction_plot <- ggplot(data = t10, mapping = aes(x = x, y = y)) +
  geom_point() +
  geom_line(aes(y = pred, col = k), size = 1) +
  labs(title = "Both models fit the data quite well",
       subtitle = "Except close to the boundaries, is the model with k = 9 really better?",
       color = "Polynomial Degree")
prediction_plot
...
mse_penalized <- function(y, pred, k, lambda = 0){
 return(mse(y,pred) + lambda * k) 
}

my_mse_penalized <- function(df, k, lambda){
  res <- tibble(
    loss = mse_penalized(df$y, df$pred, k, lambda),
    lambda = lambda
  )
  return(res)
}  
...
model_mse_penalized <- model_pred_resid %>%
  group_by(k, split) %>%
  nest() %>%
  mutate(errors = map(data, ~ my_mse_penalized(.x, k, lambda))) %>%
  unnest(errors) %>%
  select(-data)
model_mse_penalized
...
## Do not modify this line!
t12 <- model_mse_penalized %>%
  mutate(lambda = factor(lambda))
mse_penalized_plot <- ggplot(data = t12,
                             mapping = aes(x = k, y = loss, col = split, linetype = lambda)) +
  geom_line() +
  labs(title = "A stronger penalization implies a smaller optimal degree",
       subtitle = expression(paste(lambda, " = 0 corresponds to the unpenalized case")),
       x = "Polynomial Degree",
       y = "Loss",
       color = "Split",
...
model_pred_resid_best <- model_pred_resid %>%
...
#
## Do not modify this line!
residual_plot_best <- ggplot(data = model_pred_resid_best,
                             mapping = aes(x = x, y = resid)) +
  geom_point() +
  geom_hline(yintercept = 0, size = 1) +
  facet_wrap(~k) +
  labs(title = "No clear pattern in the residuals, except maybe for k = 6",
       subtitle = "Models with k = 7 and k = 9 seem to have done their job!",
       y = "Residuals")
residual_plot_best
