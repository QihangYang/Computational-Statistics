...
df <- college %>%
  ...
  mutate(total_cost = map_dbl(total_cost, sum),
         Private = factor(Private)) %>%
  select(total_cost, Private, Top10perc, PhD)
df
...
## Do not modify this line!
g1 <- ggplot(data = df, mapping = aes(x =  Top10perc, y = total_cost)) +
  geom_point() +
...
#    # … with 767 more rows
## Do not modify this line!
m0 <- lm(data = df, total_cost ~ Top10perc) 
m0 %>% broom::tidy()

total_cost_pred <- df %>%
  add_residuals(m0, var = "resid0") %>%
  add_predictions(m0, var = "pred0")
total_cost_pred
...
## Do not modify this line!
g2 <- ggplot(data = total_cost_pred, mapping = aes(x = pred0, y = resid0)) +
  geom_ref_line(h = 0) +
  geom_point() +
...
#    10  0.0834 10.1
## Do not modify this line!
m1 <- lm(data = df, log_total_cost ~ Top10perc)
m1 %>% broom::tidy()

total_cost_pred <- total_cost_pred %>%
  add_residuals(m1, var = "resid1") %>%
  add_predictions(m1, var = "pred1")
total_cost_pred
...
## Do not modify this line!
g3 <- ggplot(data = total_cost_pred, mapping = aes(x = Top10perc, y = log_total_cost)) +
  geom_point() +
  geom_line(aes(y = pred1), col = "red") +
  labs(title = "After taking log of total cost, the log of total cost has a 
       more linear relationship with the students from top 10% colleges",
       x = "Pct. of new students from top 10% H.S.",
...
## Do not modify this line!
g4 <- ggplot(data = total_cost_pred, mapping = aes(x = Top10perc, y = resid1)) +
  geom_ref_line(h = 0) +
  geom_point() +
...
## Do not modify this line!
g5 <- ggplot(data = total_cost_pred, mapping = aes(x = Private, y = log_total_cost)) +
  geom_boxplot() +
...
#     10  0.0115    10.1
## Do not modify this line!
m2 <- lm(data = total_cost_pred, log_total_cost ~ Private * Top10perc)
m2 %>% broom::tidy()

total_cost_pred <- total_cost_pred %>%
  add_residuals(m2, var = "resid2") %>%
  add_predictions(m2, var = "pred2") 
total_cost_pred
...
## Do not modify this line!
g6 <- ggplot(data = total_cost_pred, mapping = aes(x = pred2, y = resid2)) +
  geom_ref_line(h = 0) +
  geom_point() +
...
stats_m2 <- total_cost_pred %>%
  select(log_total_cost, pred2, Private, Top10perc) %>%
...
#     10  0.108    10.0
## Do not modify this line!
m3 <- lm(data = total_cost_pred, log_total_cost ~ Private * Top10perc + poly(PhD, 2))
m3 %>% broom::tidy()

total_cost_pred <- total_cost_pred %>%
  add_residuals(m3, var = "resid3") %>%
  add_predictions(m3, var = "pred3")
total_cost_pred
...
## Do not modify this line!
g9 <- ggplot(data = total_cost_pred, mapping = aes(x = pred3, y = resid3)) +
  geom_ref_line(h = 0) +
  geom_point() +
...
r_squared_and_adjusted <- model_list %>%
  ...
  pivot_longer(-group, names_to = "Type", values_to = "Value")
...
## Do not modify this line!
g10 <- ggplot(data = r_squared_and_adjusted, mapping = aes(x = group, y = Value, fill = Type)) +
...
## Do not modify this line!
mnull <- lm(data = total_cost_pred, log_total_cost ~ 1)
mnull %>% broom::tidy()
