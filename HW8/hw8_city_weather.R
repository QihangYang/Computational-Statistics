...
temperature_full <- temperature %>%
  pivot_longer(-datetime, names_to = "city", values_to = "temperature") %>%
...
## Do not modify this line!
ny_temp <- ggplot(data = temperature_ny, 
                  mapping = aes(x = datetime, y = temperature)) +
  geom_line() +
...
## Do not modify this line!
library(modelr)
model0 <- lm(data = temperature_ny, temperature ~ poly(yday, 5))
...
## Do not modify this line!
ny_temp_fitted <- ggplot(data = temperature_ny_model0, 
                         mapping = aes(x = datetime, y = temperature)) +
...
## Do not modify this line!
cyclic <- function(yearfrac) mgcv::cSplineDes(yearfrac, seq(0, 1, 0.2))[, -5]
model1 <- lm(data = temperature_ny, temperature ~ cyclic(yearfrac))
...
ny_temp_fitted2 <- ny_temp_fitted +
  geom_line(data = temperature_ny_model1, mapping = aes(y = pred), col = "green", size = 1) +
...
## Do not modify this line!
ny_temp_residual <- ggplot(data = temperature_ny_model1,
                           mapping = aes(x = hour, y = resid)) +
  geom_boxplot() +
...
#    #   resid <dbl>
## Do not modify this line!
model2 <- lm(data = temperature_ny, temperature ~ cyclic(yearfrac) + hour) 
...
#     without overfitting.
## Do not modify this line!
ny_temp_residual2 <- ggplot(data = temperature_ny_model2, 
                            mapping = aes(x = hour, y = resid)) +
  geom_boxplot() +
  facet_wrap(~ model) +
...
my_model <- function(df) {
  model2 <- lm(data = df, temperature ~ cyclic(yearfrac) + hour) 
  return(model2)
}
...
#    # … with 86 more rows
## Do not modify this line!
test_df <- crossing(yearfrac = seq(0, 0.75, 0.25), hour = unique( temperature_full %>% pull(hour)))
test_df
...
by_city_pred <- by_city %>%
  ...
  unnest(pred) %>%
  select(-c(data, model, name)) %>%
  rename("pred" = value) %>%
  mutate(yearfrac = fct_recode(factor(yearfrac), "Winter" = "0", "Spring" = "0.25", "Summer" = "0.5", "Autumn" = "0.75")) %>%
...
## Do not modify this line!
t16 <- by_city_pred %>%
  filter(city %in% c("Miami", "Phoenix", "Vancouver"))

temperature_per_hour_season_plot <- ggplot(data = t16,
                                           mapping = aes(x = hour, y = pred)) +
  geom_point() +
