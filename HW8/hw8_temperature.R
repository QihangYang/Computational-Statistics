...
temperature_change <- temperature %>%
  filter(date == "1910-03-01" | date == "2010-03-01") %>%
  group_by(country) %>%
  arrange(year) %>%
...
## Do not modify this line!
temperature_map <- ggplot(data =temperature_change,
                          mapping = aes(map_id = country)) +
...
## Do not modify this line!
switzerland_temperature_plot <- ggplot(data = sw, mapping = aes(x = month, y = temperature)) +
  geom_boxplot() +
...
## Do not modify this line!
sw_mod <- lm(data = sw, temperature ~ month)
sw_mod %>% broom::tidy()
...
sw_seasonal_plot <- switzerland_temperature_plot +
  geom_point(data = sw_seasonal, aes(y = `mean temperature`),
             color = "green", size = 2, pch = 3) +
...
## Do not modify this line!
residuals_plot <- ggplot(data = sw_residuals,
                            mapping = aes(x = year, y = resid)) +
  geom_point() +
  geom_smooth(method = "lm", col = "red", size = 2, se = FALSE) +
...
## Do not modify this line!
sw_improved_mod <- lm(data = sw, temperature ~ year + month)  
...
country_model <- function(df){
  res <- lm(data = df, temperature ~ year + month)
  return(res)
}
...
## Do not modify this line!
monthly_pattern_plot <- ggplot(data = monthly_pattern,
                               mapping = aes(x = month, y = pred, col = country, group = country)) +
  geom_point() +
  geom_line() +
