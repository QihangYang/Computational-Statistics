...
#        #   renovated <fct>
## Do not modify this line!
check_na <- map_lgl(data, ~ any(is.na(.x)))

data_clean <- data %>%
  mutate(quarter = factor(quarter(date)),
         age = 2019 - yr_built,
         renovated = fct_recode(factor(yr_renovated != 0), "No" = "FALSE", "Yes" = "TRUE"),
         waterfront = fct_recode(factor(waterfront), "No" = "0", "Yes" = "1")) %>%
...
#    side-by-side.
## Do not modify this line!
g1 <- ggplot(data = data_clean, mapping = aes(x = waterfront, y = price)) +
  geom_boxplot() +
  coord_flip() +
...
g2 <- ggplot(data = data_clean, mapping = aes(x = renovated, y = price)) +
  geom_boxplot() +
  coord_flip() +
...
## Do not modify this line!
g3 <- ggpairs(data = data_clean[c("price", "sqft_lot", "sqft_above", "sqft_living", "sqft_basement")]) +
...
data_model <- data_clean %>%
  select(-c(quarter, zipcode, sqft_above, sqft_basement, sqft_living15, sqft_lot15, sqft_lot)) 

m1 <- lm(data = data_model, log10(price) ~ .)

fit_m1 <- data_model %>%
  add_residuals(m1, var = "resids") %>%
  add_predictions(m1, var = "preds") %>%
  select(resids, preds) 
...
## Do not modify this line!
g4 <- ggplot(data = fit_m1, mapping = aes(x = preds, y = resids)) +
...
## Do not modify this line!
m2 <- lm(data = data_no_outlier, log10(price) ~ . + lat * long)

fit_m2 <- data_no_outlier %>%
  add_residuals(m2, var = "resids") %>%
  add_predictions(m2, var = "preds") %>%
  select(resids, preds)
...
## Do not modify this line!
glance <- glance(m1) %>%
  bind_rows(glance(m2), .id = "model")


g6 <- ggplot(data = fit_m2, mapping = aes(x = preds, y = resids)) +
  geom_point(alpha = 0.3) +
  geom_line(y = 0, col = "red") +
...
by_quarter <- data_clean %>%
  select(-c(zipcode, sqft_above, sqft_basement, sqft_living15, sqft_lot, sqft_lot15)) %>%
...
## Do not modify this line!
f <- function(df){
  m2 <- lm(data = df, log10(price) ~ . + lat * long)
  return(m2)
}
...
by_quarter_resid <- by_quarter %>%
  unnest(resids) %>%
  mutate(key = row_number()) %>%
  select(quarter, key, resid) 


by_quarter_pred <- by_quarter %>%
  unnest(preds) %>%
  mutate(key = row_number()) %>%
  select(quarter, key, pred) 


by_quarter_fit <- by_quarter_resid %>%
  inner_join(by_quarter_pred)
...
## Do not modify this line!
g8 <- ggplot(data = by_quarter_fit, mapping = aes(x = pred, y = resid)) +
  geom_point(alpha = 0.3) +
