...
spotify <- data %>%
  mutate(popularity_label = 
           fct_recode(factor(cut(popularity, breaks = c(0, 30, 70, 100))),
...
danceability_dist <- ggplot(data = spotify) +
  geom_histogram(aes(x = danceability, y = ..density..), binwidth = 0.04, 
...
qq_plot_tempo <- ggplot(data = spotify_bts, mapping = aes(sample = tempo)) +
...
par_gamma <- function(x){
  ...
  res <- optim(par0, nll_gamma_x, lower = 0, method = "L-BFGS-B")$par
  return(res)
}
par_gamma(c(1,2))
...
liveness_gamma_plot <- ggplot(data = spotify_pop) +
  geom_histogram(aes(x = liveness, y = ..density..), binwidth = 0.02, 
                 color = "black", fill = "orange") +
  geom_density(aes(x = liveness), kernel = "gaussian", bw = 0.06, color = "red",  fill = "red", alpha = 0.5) +
