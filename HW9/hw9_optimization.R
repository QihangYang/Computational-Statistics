...
ca <- function(x) {
  return(40 * 60 / 24 - 50 / 24 * x)
}

cb <- function(x) {
  return(35 * 60 / 33 - 30 / 33 * x)
}


g1 <- ggplot(data = x) +
  stat_function(fun = ca) +
  ...
  geom_point(mapping = aes(x = 45, y = 5), col = "red") +
  geom_point(mapping = aes(x = 45, y = 6.25), col = "red") +
  geom_point(mapping = aes(x = 45.6, y = 5), col = "red") +
...
g2 <- g1 +
  stat_function(fun = line1, col = "blue") +
  stat_function(fun = line2, col = "blue") +
  stat_function(fun = line3, col = "blue")
g2
...
f.obj <- c(1, 1)
f.con <- matrix(
  data = c(-50, -24, -30, -33, 1, 0, 0, 1), 
  nrow = 4, ncol = 2, byrow = TRUE
)
f.dir <- ">="
f.rhs <- c(-40*60, -35*60, 45, 5)

s1 <- lp(direction = "max", f.obj, f.con, f.dir, f.rhs)
s1 
...
f.obj <- c(18, 25, 10, 12, 15)
f.con <- matrix(
  data = c(1.2, 1.3, 0.7, 0, 0.5, 
           0.7, 2.2, 1.6, 0.5, 1,
           0.9, 0.7, 1.3, 1, 0.8,
           1.4, 2.8, 0.5, 1.2, 0.6),
...
f.dir <- "<="
f.rhs <- c(40*4, 40*5, 40*3, 40*7)

s2 <- lp("max", f.obj, f.con, f.dir, f.rhs, all.int = TRUE)
s2
