...
f <- function(x) {
  return(exp(x*x/(x*x + 2*x + 5))*tanh(x)*sin(x)/(x*(1 + .5/abs(x))))
}
...
library(dplyr)
library(ggplot2)
f_plot <- tibble(x = seq(-30, 30, by = 0.01)) %>%
  mutate(y = f(x)) %>%
...
one_step <- function(f, x, h) {
  res <- x - f(x) / ((f(x + h) - f(x)) / h)
  return(res)
}
multi_step <- function(f, x, h, n_iterations) {
  res <- enframe(accumulate(rep(h, n_iterations), one_step, f = f, .init = x), name = "step", value = "x")
  return(res)
}
multi_step(f, 1, h, 2)
...
## Do not modify this line!
library(tidyr)
newton_steps_tibble <- tibble(
  start = seq(-7, 7, 2),
  steps = map(start, ~ multi_step(f, .x, h, n_iterations = 2)) 
)%>%
  unnest(steps) %>%
  mutate(f = f(x),
         step = factor(step),
...
#    - add `theme_light()`
## Do not modify this line!
newton_steps_plot <- ggplot(data = newton_steps_tibble,
                            mapping = aes(x = x, y = f)) +
  stat_function(fun = f) +
  geom_point(aes(col = step), size = 2) +
  geom_hline(yintercept = 0, alpha = 0.2) +
  facet_wrap( ~ start) +
  labs(title = "The points get closer to roots of f as steps increase",
       y = "f(x)") +
  theme_light()
newton_steps_plot
...
plot_with_root <- f_plot +
  geom_point(aes(x = root, y = f(root)), color = "red") +
