...
obj_lnorm_factory <- function(x) {
  force(x)
  function(par){
    res <- sum((c(mean(x), var(x)) - mom_lnorm(par))^2)
    return(res)
  }
}
...
par_lnorm <- function(x) {
  mu_mom <- optim(c(1,1), obj_lnorm_factory(x), hessian = TRUE)$par[1]
  sd_mom <- optim(c(1,1), obj_lnorm_factory(x), hessian = TRUE)$par[2]
  hessian <- optim(c(1,1), obj_lnorm_factory(x), hessian = TRUE)$hessian
  sd <- sqrt(diag(solve(hessian)))
  res <- tribble(
    ~parameter, ~value, ~se, ~lower, ~upper,
    "mu_mom", mu_mom, sd[1], mu_mom - 1.96 * sd[1], mu_mom + 1.96 * sd[1],
    "sd_mom", sd_mom, sd[2], sd_mom - 1.96 * sd[2], sd_mom + 1.96 * sd[2]
  )
  return(res)
}
...
nll_lnorm <- function(par, x) {
  return(-sum(dlnorm(x, par[1], par[2], log = TRUE)))
}

mu_mle <- optim(c(1,1), nll_lnorm, x = x)$par[1]
sd_mle <- optim(c(1,1), nll_lnorm, x = x)$par[2]
...
library(ggplot2)
x <- as.data.frame(x)
compare_plot <- ggplot(data = x, mapping = aes(x)) +
  geom_histogram(aes(y = ..density.., color = "Data"), bins = 50, ) +
