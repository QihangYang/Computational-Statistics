...
stock_return_plot <- ggplot(data = stocks_data_longer, 
                            mapping = aes(x = date, y = return, group = stock)) +
...
dvec <- rep(0, 10)
A.Equality <- rep(1, 10)
Amat <- rbind(A.Equality, colMeans(stocks_data[-1]), diag(10), -diag(10))
...
## Do not modify this line!
portfolio_plot <- ggplot(data = portfolio, mapping = aes(x = stock, y = proportion)) +
