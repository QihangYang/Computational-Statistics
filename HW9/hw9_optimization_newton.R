...
find_cost <- function(X, y, theta) {
  res <- mean( -y * log(sigmoid(X %*% theta)) - (1 - y) * log(1 - sigmoid(X %*% theta)))
  return(res)
}
...
logit <- glm(data = data, admit ~ gre + gpa, family = "binomial")
