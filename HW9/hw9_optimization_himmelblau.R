...
himmelblau <- function(para) {
  return(himmelblau_helper(para[1], para[2]))
}
...
#    4 x 4 square.
## Do not modify this line!
himmelblau_plot <- wireframe(fnxy ~ x * y, data = xy, shade = TRUE, scales = list(arrows = FALSE), 
...
## Do not modify this line!
library(viridis)
contour <- ggplot(data = xy, mapping = aes(x = x, y = y, z = fnxy)) +
...
softline_dir <- crossing(x = c(1, -1), y = c(1, -1)) %>%
  mutate(d0 = map2(x, y, c),
         a = map_dbl(d0, ~ softline(x0, .x, himmelblau)),
         optimum = map2(d0, a, ~ (x0 + .y * .x)),
...
compare_plot <- contour +
  geom_point(data = pts, mapping = aes(color = method), size = 2) +
