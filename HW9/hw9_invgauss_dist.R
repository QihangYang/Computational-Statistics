...
mom_invgauss <- function(par){
  moments <- vector(length = 2)
  moments[1] <- par[1]
  moments[2] <- par[1] ^ 3 / par[2]
  return(moments)
}

obj_invgauss <- function(par, x) {
  moments <- c(mean(x), var(x))
  estmoments <- mom_invgauss(par)
  differences <- moments - estmoments
  res <- sum(differences ^ 2)
  return(res)
}

par_invgauss1 <- function(x) {
  res <- optim(c(1,1), obj_invgauss, x = x)
  return(res$par)
}
...
g1 <- ggplot(data = random_data_tbl, mapping = aes(sample = random_data)) +
  geom_qq(distribution = qinvgauss, dparams = dparams) +
  stat_qq_line(distribution = qinvgauss, dparams = dparams, col = "red") +
...
nll_invgauss <- function(par, x) {
  res <- -sum(dinvgauss(x, mean = par[1], shape = par[2], log = TRUE))
  return(res)
}
...
fit <- optim(c(1,1), nll_invgauss, x = random_data, hessian = TRUE, lower = 0, method = "L-BFGS-B")
...
## Do not modify this line!
ks.test(random_data, pinvgauss, mean = mu_hat, shape = shape_hat)
...
mydinvgauss <- function(par) {
  force(par)
  function(x){
    return(dinvgauss(x, par[1], par[2]))
  }
}

par0 <- c(mean(random_data), var(random_data))

g2 <- ggplot(data = random_data_tbl, mapping = aes(x = random_data)) +
  geom_histogram(aes(y = ..density..), bins = 15) +
  geom_density(aes(color = "Data")) +
  stat_function(aes(color = "MOM"), fun = mydinvgauss(par1)) +
  stat_function(aes(color = "MLE"), fun = mydinvgauss(par2)) +
  stat_function(aes(color = "True Value"), fun = mydinvgauss(par0)) +
