...
alpha0 <- mean(x) * (mean(x) * (1 - mean(x)) / var(x) - 1)

beta0 <- (1 - mean(x)) * (mean(x) * (1 - mean(x)) / var(x) - 1)
...
mom_beta <- function(par = vector(length = 2)){
  res <- c(par[1] / (par[1] + par[2]), 
           par[1] * par[2] / ((par[1] + par[2]) ^2 * (par[1] + par[2] + 1))) 
  return(res)
}
...
obj_beta_factory <- function(x) {
  force(x)
  function(par){
    res <- (mean(x) - mom_beta(par)[1])^2 + (var(x) - mom_beta(par)[2])^2
    return(res)
  }
}
...
## Do not modify this line!
par_beta <- function(x) {
  obj <- obj_beta_factory(x) 
  res <- optim(c(1, 1), obj)$par
  return(res)
}
...
nll_beta <- function(par, x){
  res <- -sum(dbeta(x, par[1], par[2], log = TRUE))
  return(res)
}
...
alpha2 <- op$par[1]
beta2 <- op$par[2]
lower <- op$par - se * 1.96
upper <- op$par + se * 1.96
...
g1 <- ggplot(data = x, mapping = aes(x = x)) +
  ...
  stat_function(aes(col = "MOM"), fun = mydbeta(alpha1, beta1)) +
  stat_function(aes(col = "MLE"), fun = mydbeta(alpha2, beta2)) +
  stat_function(aes(col = "True Value"), fun = mydbeta(alpha0, beta0)) +
