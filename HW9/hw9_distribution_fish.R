...
## Do not modify this line!
g1 <- ggplot(data = data, mapping = aes(x = count)) +
  geom_histogram() +
...
## Do not modify this line!
g2 <- ggplot(data = data, mapping = aes(x = count)) +
  stat_ecdf() +
...
nll_pois_factory <- function(x){
  ...
  function(par){
    lambda = par[1]
    pi = par[2]
    res <- y * log(pi + (1 - pi) * exp(-lambda)) + (n - y) * log(1 - pi) - (n - y) * lambda + 
                     n * xbar * log(lambda)
    return(-res)
  }
}
...
## Do not modify this line!
cnt_pois_mle <- fit_mle %>%
  pull(value)

dparams_mle <- list(
  mu = cnt_pois_mle[1],
  sigma = cnt_pois_mle[2]
)

g3 <- ggplot(data = data, mapping = aes(sample = count)) +
