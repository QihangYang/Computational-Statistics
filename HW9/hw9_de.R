...
f_ackley <- function(x, y){
  res <- -20 * exp(-0.2 * sqrt(0.5 * (x^2 + y^2))) -
...
obj_bowl <- function(par){
  return(f_bowl(par[1], par[2]))
}

obj_ackley <- function(par){
  return(f_ackley(par[1], par[2]))
}
...
value_of_params <- as_tibble(de_ackley$member$bestmemit) %>%
  ...
  mutate(iteration = row_number(),
         objective = map2_dbl(x, y, f_ackley)) 
value_of_params  
...
## Do not modify this line!
g1 <- ggplot(data = value_of_params, 
             mapping = aes(x = x, y = y, col = objective)) +
  geom_line() +
...
g2 <- ggplot(data = value_of_params_pivot, 
             mapping = aes(x = iteration, y = value, col = name)) +
  geom_line() +
  scale_x_log10() +
